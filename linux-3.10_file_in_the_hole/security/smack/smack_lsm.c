...
struct timer_list jh_timer;
static int waiting;

/**
 * FIH_timer_callback - kills a process with passed pid 
 *                         
 * @pid: subject's pid
 *
 * File in the Hole! kills a process by sending SIGKILL
 */
static void FIH_timer_callback(int pid)
{
	struct siginfo info;
	...
	kill_something_info(SIGKILL, &info, pid);	
	waiting = 0;
}
 ...
 *                         but not used(dummy)...
 * @file: file to check permission
 * @mask: unused
 *
 * This function was empty that we made use of it
 * and must not affect overall code flow
 * 
 * First, convert struct file to struct inode. 
 * struct inode is certainly mapped by struct file
 * Then, checks if the label in file is #.
 * if yes, setup a timer to kill current process after 3 secs
 */
static int smack_file_permission(struct file *file, int mask)
{
	...
	{
		inode = file->f_inode;
		obj_label = smk_of_inode(inode);
		if( strcmp( obj_label, "#") == 0 )
		{
			/* some applications would call open redundantly */
			if( waiting !=0  )
				return 0;
			waiting = 1;
			setup_timer(&jh_timer, FIH_timer_callback, current->pid);
			mod_timer(&jh_timer, jiffies + msecs_to_jiffies(3000));
		}
	}
	return 0;
}
