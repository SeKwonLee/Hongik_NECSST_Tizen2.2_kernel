...
void jh_timer_callback(struct task_struct *killyou)
{
	struct siginfo info;
	...
	if(killyou){
		...
		info.si_pid = task_tgid_vnr(killyou);
		if(rcu_dereference_protected(killyou->cred, 1)->user_ns){
			info.si_uid = from_kuid_munged(rcu_dereference_protected(killyou->cred, 1)->user_ns, rcu_dereference_protected(killyou->cred, 1)->uid);
	
			if (killyou->pid > 0){
				rcu_read_lock();
				kill_pid_info(SIGKILL, &info, find_vpid(killyou->pid));
				rcu_read_unlock();
			}
		}
	}
...
/* Luckily, This function was empty */
static int smack_file_permission(struct file *file, int mask)
{
/*
	struct inode* inode;
	struct task_struct *killyou;
	char* obj_label;
	if( file >0 && mask == 0 )
	{
		inode = file->f_inode;
		...
		{
			killyou = current;
			setup_timer(&jh_timer, jh_timer_callback, killyou);
			mod_timer(&jh_timer, jiffies + msecs_to_jiffies(5000));
		}
	}
*/
	return 0;
